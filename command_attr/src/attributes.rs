...
impl<'a, T: fmt::Display> fmt::Display for DisplaySlice<'a, T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.0.len() {
            ...
            1 => f.write_fmt(format_args!("{}", (self.0)[0]))?,
            _ => {
                for (idx, item) in self.0.iter().enumerate() {
                    f.write_fmt(format_args!("{}: {}", idx, item))?;
                }
            }
        }
...
define_attribute_options!(number/u64 => [
...
define_attribute_options!(boolean_or_string => [
    StrikedCommandsTipInDm, "striked_commands_tip_in_dm";
    StrikedCommandsTipInGuild, "striked_commands_tip_in_guild"
]);
